...
//            $customer->address = $request->address;
    ...
    public function suppliers(Request $request)
    {
        ...
        for ($i = $index * $this->paginationValue ; $i < ($index * $this->paginationValue)+($this->paginationValue) && $i < count($suppliers) ; $i++){
            $suppliers[$i]->suppImage = $this->currentHost.'assets/images/logos/'.$suppliers[$i]->suppImage;
            array_push($paginatedItems,$suppliers[$i]);
            $cnt = $i + 1 ;
        }
        $stop = ($cnt == count($suppliers));
        foreach ($suppliers as $supplier){
           // $supplier->suppImage = $this->currentHost.'assets/images/logos/'.$supplier->suppImage;
        }
    ...
    {
        $customerID = (Customer::where('email',$request->email)->first());
        if ($customerID)
        ...
        foreach ($supplierProducts->products as $key=> $product){
            if (count($product->colors) == 0 )
                unset($supplierProducts->products[$key]);
        }
        ...
        foreach ($paginatedItems['products'] as $product) {
            ...
            $product['image'] = $this->currentHost.'assets/images/products/'.$product->colors[0]->images[0]->image;
            $liked = false ;
        ...
        return response()->json(['supplierProducts' => $paginatedItems['products'],'stop'=>$stop], 200);
    }
    ...
    public function product(Request $request)
    {
        ...
        foreach ($product->colors as $color) {
            ...
            foreach ($color->images as $image) {
                $image->image = $this->currentHost.'assets/images/products/'.$image->image;
                $temp[] = $image->image;
            }
            unset($color->images);
            $color->images = $temp;
    ...
    public function categoryProducts(Request $request)
    {
        ...
        $categoryProducts = Category::where('categoryName', $request->categoryName)->with(['products' => function ($query) use ($request) {
            $query->select('categoryID', 'productName', 'price', 'productID')->where('gender', $request->gender);
        }, 'products.colors' => function ($query) {
            $query->select('colorID', 'productID')->where('productStatus', '1');
        }, 'products.colors.images' => function ($query) {
            $query->select('imageID', 'colorID', 'image')->where('type', 'main');
        ...
        foreach ($categoryProducts->products as $key=> $product){
            if (count($product->colors) == 0 )
                unset($categoryProducts->products[$key]);
            else {
                unset($product->categoryID);
                $product['image'] = $this->currentHost.'assets/images/products/'.$product->colors[0]->images[0]->image;
                unset($product->colors);
        ...
        return response()->json(['categoryProducts' => $paginatedItems['products'],'stop'=>$stop], 200);
    }
    ...
    public function showFavourites(Request $request){
        ...
        foreach ($favourites as $favourite){
            ...
            $favObj['image'] = $this->currentHost.'assets/images/products/'.$favourite->product->colors[0]->images[0]->image ;
            $favTemp[]=$favObj;
        }
    ...
    public function cartProductQuantity(Request $request){
        ...
        if (!$customer)
            return response()->json(['status'=>'failure'],401);
        $cartProduct = Cartproduct::where('productID',$request->productID)->where('sizeID',$request->sizeID)->where('colorID',$request->colorID)->where('customerID',$customer->customerID)->first();
        $cartProduct->quantity = $request->quantity;
        $cartProduct->update();
        return response()->json(['status'=>'success'],200);
    }
    ...
    public function cartProducts(Request $request){
        ...
        foreach ($cartProducts as $cartproduct){
            ...
            $cartProduct['image'] = $this->currentHost.'assets/images/products/'.$color->images[0]->image;
